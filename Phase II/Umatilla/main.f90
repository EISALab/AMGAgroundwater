...
implicit none
	if( hUmaModel/=0 )then
		call release_meta_model( hUmaModel )
!		call release_net_cluster(hUmaNet)
	endif
	if( hCostModel/=0 )then 
		call release_meta_model( hCostModel )
!		call release_net_cluster(hCostNet)
	endif
