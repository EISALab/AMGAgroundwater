...
integer function CalcRemYear( cmax, cl )
implicit none
	double precision, intent(in) :: cmax, cl
	if(abs(max(cmax,cl)/cl-1.) .le. 0.05 ) then
		CalcRemYear = 4
	else
		CalcRemYear = 5
	endif
...
implicit none
	...
	if( rReliab>=1 )then
		...
		ComputeReliabPen = (1.0 - rReliab )*pen_wt
	endif
end function

!compute Rdx and Tnt violations by Penalties returned from surrga.
subroutine ComputeRdxTntViols( Pen_Rdx, Pen_Tnt, rViolRdx, rViolTnt )
USE PARAMS
implicit none
	double precision, intent(out) :: rViolRdx, rViolTnt
	...
	if( Pen_Rdx > 0 )then
		rViolRdx = Pen_Rdx / pen_wt_Rdx
	else
		rViolRdx = 0
	endif

	if( Pen_Tnt > 0 )then
		rViolTnt = Pen_Tnt / pen_wt_Tnt
	else
		rViolTnt = 0
	endif
end subroutine
...
!important--
!costbyNN doesn't compute the VCG cost. this cost component is predicted by neural networks.
subroutine CostByNN(c_max_rdx, c_max_tnt, rDummyVCGCost, rFourMaxRdxC, rFourMaxTntC, Fitness, Total_cost, Pen_Rdx, Pen_Tnt, gridflagx, gridflagy,drycell_c,drycell_h)
  !*********************************************************************
...
!  integer :: FormulNum, RchLoc, Numy, Ny_Rdx, Ny_Tnt, Ny
  integer :: Ny_Rdx, Ny_Tnt, FlgCleanup
  integer :: FormulNum, RchLoc, Numy, Ny
!  integer :: FlgCleanup,PeriodEw2, NumNewWell, NumNewBasin,CountConstr
  integer :: PeriodEw2, NumNewWell, NumNewBasin,CountConstr
...
!  real :: WelConc(MxYr1,MxWel1), PlumeArea(Nstress), PlumeMass(MxYr1)
  real :: PlumeArea(Nstress), PlumeMass(MxYr1)
!  real :: CRdx(OriNrow1,OriNcol1,MxYr1), CTnt(OriNrow1,OriNcol1,MxYr1), V_Rdx, V_Tnt
  ...
  double precision, intent(in) :: c_max_rdx, c_max_tnt, rDummyVCGCost
  double precision, intent(in) :: rFourMaxRdxC, rFourMaxTntC
...
!          write(iout,'(/1x,''The cleanup year for TNT: '',i5)') Ny_Tnt
      ...
      ! ------------
      Ny = Numy
	  Ny_Rdx = CalcRemYear( rFourMaxRdxC, dble(Cl_Rdx) )
	  Ny_Tnt = CalcRemYear( rFourMaxTntC, dble(Cl_Tnt) )
!commented by Shegnguqna Yan, not using them for nn cost
...
15    format(/5x,'Number of GACs Installed in Each Stress Period')
 ...
 !Commented by Shengquan Yan, because Nstress is 2, this loop is never executed
     do K = 2,Nstress
         Year = (K-1)*5
...
28        format(/5x,'The Variable Costs of Sampling (thousand of dollars)'&
          ...
          ! -----------------------------------------
      Total_cost = dble(CCW) + dble(CCB) + dble(CCG) + dble(FCL) + dble(FCE) + dble(VCE) + dble(VCG) + dble(VCS) + dble(rDummyVCGCost)

	if( c_max_rdx > Cl_Rdx )then
		V_Rdx = c_max_rdx / Cl_Rdx - 1.0
	else
		V_Rdx = 0.0
	endif
	if( c_max_tnt > Cl_Tnt )then
		V_Tnt = c_max_tnt / Cl_Tnt - 1.0
	else
		V_Tnt = 0.0
	endif
