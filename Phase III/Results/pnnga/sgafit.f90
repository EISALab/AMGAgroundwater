...
integer, parameter :: MAX_SAMPLES = 200
...
double precision function ComputeReliabPen( rReliab )
implicit none
	double precision, intent(in) :: rReliab

	double precision :: pen_wt;

	pen_wt = 5000.0
	if( rReliab>=0.9 )then
		ComputeReliabPen = 0.0
	else
		ComputeReliabPen = (0.9 - rReliab )*pen_wt
	endif
end function
...
implicit none
	...
	double precision, dimension(4*g_nRemWells+3) :: arrVars			!the vars for parsing and normalize the NN parameters.
	double precision, dimension( g_nRemWells ) :: arrHeads
	double precision :: rRiskB, rRiskW, rReliab
	...
	do i=1, sgaParam%nPopSize
		...
		!the reliability 
		rReliab = arrVars(4*g_nRemWells+3)
		if( rReliab<0 )rReliab = 0
		if( rReliab>1 )rReliab = 1.0
		sgaParam%arrReliabs(i) = rReliab
		...
		sgaParam%arrFit(i) = g_rFitness + ComputeReliabPen( rReliab )
...
implicit none
	...
	do i = 1, nIds
		...
		if( SearchChromCache(hChromCache, sgaParam%arrPop(id,:), sgaParam%nChromBits, -1, arrCachedFits, nCachedFits)/=0 )then
			...
			sgaParam%arrFit(id) = arrCachedFits(3) + ComputeReliabPen( arrCachedFits(nCachedFits) )
			sgaParam%arrFitStds(id) = arrCachedFitStds(3)

			sgaParam%arrReliabs(id) = arrCachedFits(nCachedFits)
...
implicit none
	...
	do i=lbound(sgaParam%arrFitInj, 1), ubound(sgaParam%arrFitInj,1)
		...
		if( SearchChromCache(hChromCache, sgaParam%arrPopInj(i,:), sgaParam%nChromBits, -1, arrCachedFits, nCachedFits)/=0 )then
			...
			sgaParam%arrFitInj(i) = arrCachedFits(3) + ComputeReliabPen( arrCachedFits(nCachedFits) )
			...
			sgaParam%arrReliabsInj(i) = arrCachedFits(nCachedFits)
		endif
	enddo
end subroutine
...
contains
	...
	implicit none
		...
		rRiskB = arrCachedFits( index+2*g_nRemWells )
		rRiskW = arrCachedFits( index+2*g_nRemWells+1 )
		rReliab = arrCachedFits( nCachedFits )
		...
		!write the risk training data into file
		call ParseVars( arrVars, arrHeads, rRiskB, rRiskW, rReliab )
		call Normalize( handles%hRiskMeanNorm, arrVars, 1 )
		call OrderVars( arrVars, arrWellIndex )
		call AppendToFile( strRiskFile, arrVars, 4*g_nRemWells+3 )
...
!	call qsort( arrIds, sgaParam%nPopSize, sizeof(arrIds(1)), CompareByFitness )
	...
	do i=1, sgaParam%nPopSize
		...
		call SaveIndInfo( id, sgaParam%arrFlags(id), samples, rReliab, arFit, arCost, arViol, arTotCost, arRiskB, arRiskW, &
...
contains
	subroutine SaveIndInfo( id, nFlag, samples, rReliab, arFit, arCost, arViol, arTotCost, arRiskB, arRiskW, &
	...
	write(hGaOut,fmt=1100,advance="NO")rReliab, arFit, arCost, arViol, arTotCost, arRiskB, arRiskW
	...
	1100 format( 2x, f5.3, 2x, 4(f17.6, '(', f14.6, ')'), 2(f15.12, '(', f15.12, ')') )
	1200 format( 2x, i2)
	1300 format(2x, f9.4)
...
implicit none                                                                                                                                                                                             
	...
	double precision :: rCost, rViols, rReliab
	...
	nCachedFits = 4 + 2*g_nRemWells + 2	+ 1	!cost, viols, fitness, totcost, heads(:), oldheads(:), riskb, riskw, rReliab
...
!			call qsort( arrIndex, sgaParam%nPopSize, sizeof(arrIndex(1)), FitCompare )
	...
	do i=1, nJobCount
		...
		arrCachedFits = (/rCost, rViols, g_rFitness, g_rTotCost, g_arrHeads, g_arrOldHeads, g_rRiskB, g_rRiskW, rReliab /)
