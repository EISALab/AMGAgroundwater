...
integer, dimension(3) :: arrRiskNet = (/12, 15, 3/)
...
implicit none
	...
	!variables
	double precision, dimension(4*g_nRemWells+3) :: arrOrigMin, arrOrigMax			!(x,y), head, riskb, riskw.
	double precision, dimension(4*g_nRemWells) :: arrHeadNormMin, arrHeadNormMax
	double precision, dimension(4*g_nRemWells+3) :: arrRiskNormMin, arrRiskNormMax
	integer :: i
	...
	arrRiskNormMin = (/(-1.0, i=1,4*g_nRemWells), -0.9, -0.9, -0.9/)
	arrRiskNormMax = (/(1.0, i=1,4*g_nRemWells), 0.9, 0.9, 0.9/)

	call PrepareScallingRange(arrOrigMin, arrOrigMax, 4*g_nRemWells+3)
	normHead = CreateNormalize( arrOrigMin, arrOrigMax, arrHeadNormMin, arrHeadNormMax, 4*g_nRemWells )
	normRisk = CreateNormalize( arrOrigMin, arrOrigMax, arrRiskNormMin, arrRiskNormMax, 4*g_nRemWells+3 )
end subroutine
...
!	call assert( nCounts >= GetHeadNNInpCount()+GetHeadNNOutCount() )
	...
	!rReliab
	arrMax(4*g_nRemWells+3) = 1.0
	arrMin(4*g_nRemWells+3) = 0.0
...
subroutine ParseVars( arrVars, arrHeads, rRiskB, rRiskW, rReliab )
implicit none
	!arguments                                                                                                                                                                                
	double precision, dimension(:) :: arrVars
	double precision, dimension(:), optional, intent(in) :: arrHeads
	double precision, optional, intent(in) :: rRiskB, rRiskW, rReliab
	...
	if( present(rRiskB ) ) then
		...
		arrVars(4*g_nRemWells+3) = rReliab
	endif
end subroutine
