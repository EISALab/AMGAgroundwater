...
bool CClientSession::Open()
{
	if( m_pClntProxy->OpenChannel(this) ){
		...
		if( pAck!=NULL && pAck->IsSucceeded() ){
			...
			//hook up for tunneled messages, the Acked server address is the server behind the firewall.
			m_addrServ.m_addrHost = m_pServToken->GetServAddr().m_addrHost;
...
SOCKET CClientSession::OpenStream( u_int nPasPort )
{
	...
	int yes = 1;
	setsockopt(sock_stream, SOL_SOCKET, SO_REUSEADDR, (char*)&yes, sizeof(yes));
...
/*	CMessageBase msg;
	...
	//send the file stream
//Note. I have to disable the shutdown, otherwise the proxy server can't relay the stream message correctly when tunneled by ssh
...
//			cdump<<"connecting ... "<<GetHostByAddr( srvAddr.sin_addr ).c_str()<<endl;
