...
//		cdump<<lock<<buf<<" closed"<<endl<<unlock;
		...
		//notify the manager that this tocken has socket error.
		pSession->GetToken()->SetDead(CTS_E_SOCK);
...
void CJobSet::RouteJobs( CClientManager* pManager )
{
	...
	cdump<<lock<<endl<<asctime(timeinfo)<<"  start jobs!"<<endl<<unlock;
...
//		}
	...
	char buf_time[50], buf[512];
	_strtime( buf_time );
	string strHost = GetHostByAddr( pSession->GetServAddr().m_addrHost );
	sprintf( buf, "At %s - %s(%d)\tjob %d done:%d", buf_time, strHost.c_str(), pSession->GetServAddr().m_nServId, pJobObj->GetId(), bOk );
