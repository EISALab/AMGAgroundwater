     ...
     1    NLAY,NODES,INBAS,IOUT,IUNIT,CUNIT,INUNIT,IXSEC,ICHFLG,
     1    IFREFM,PATH)
C
...
C     ------------------------------------------------------------------
      ...
      CHARACTER*80 LINE1,LINE2,PATH
C     ------------------------------------------------------------------
C0------OPEN FILES AND ASSIGN IUNIT VALUES.
C-----EMRL COMMENT - ADD PATH TO SMAS50 FUNCTION
      CALL SBAS5O(INUNIT,INBAS,IOUT,IUNIT,CUNIT,PATH)
...
C     ------------------------------------------------------------------
     ...
     2          BUFF(NCOL,NROW,NLAY)
      ...
      INTEGER i,j,k  ! added by meghna....sept 21, 2001
C     ------------------------------------------------------------------
C
...
Chnages made by Meghna Babbar........21 sept 2001

  !    call headnewout(HNEW,NLAY,NCOL,NROW)
  !    call headoldout(STRT,NLAY,NCOL,NROW)
      OPEN(UNIT= 333, FILE='headnew.out',STATUS='UNKNOWN')
      REWIND 333 
          do i=1,NLAY
            do j=1,NROW
              do k=1,NCOL
                write(333,*) HNEW(k,j,i)
              end do
            end do
          end do
       close(333)

      OPEN(UNIT= 331, FILE='headold.out',STATUS='UNKNOWN')
      REWIND 331 
       do i=1,NLAY
        do j=1,NROW
         do k=1,NCOL
           write(331,*) STRT(k,j,i)
         end do
        end do
       end do
...
C5------RETURN.
      RETURN
      END
      SUBROUTINE SBAS5O(INUNIT,INBAS,IOUT,IUNIT,CUNIT,PATH)
C
...
C     ------------------------------------------------------------------
      DIMENSION IUNIT(40)
      CHARACTER*4 CUNIT(40)
      CHARACTER*80 LINE,PATH !,FLNAME
	! a bug patched by Shengquan Yan 10/25/02
...
C
C
C-----EMRL COMMENT - CHECK FOR "MT3D" FILE TYPE.
      IF(LINE(ITYP1:ITYP2).EQ.'MT3D') FMTARG='UNFORMATTED'
C
...
C-----EMRL COMMENT - ADD SETPATH_MF TO HANDLE PATH ISSUES
	   CALL SETPATH_MF(PATH,FLNAME)
         OPEN(UNIT=IU,FILE=FLNAME,FORM=FMTARG,
     1        ACCESS='SEQUENTIAL')

C         OPEN(UNIT=IU,FILE=LINE(INAM1:INAM2),FORM=FMTARG,
