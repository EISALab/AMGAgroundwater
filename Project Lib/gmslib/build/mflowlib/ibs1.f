...
C   IBQ=0 and IBQ>0.
     ...
     1                  NCOL,NROW,NLAY,IIBSCB,IIBSOC,ISS,IN,IOUT,ifrefm)
C
...
C4------CRITICAL HEAD ARRAYS.
  100 if (ifrefm.eq.0) then  !emrl ff
        READ(IN,3) IIBSCB,IIBSOC
    3   FORMAT(2I10)
      else   !emrl ff
        read(in,'(a)') line                                     !emrl ff
        lloc=1                                                  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,iibscb,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,iibsoc,R,IOUT,in)  !emrl ff
      endif  !emrl ff
C
...
C10-----RETURN.
     ...
     2                  ISUBUN,ICOMUN,IHCUN,IN,IOUT,ifrefm)
C
...
C5------INITIALIZE AND READ OUTPUT FLAGS.
  ...
  100   FORMAT(6I10)
        ...
        read(in,'(a)') line                                     !emrl ff
        lloc=1                                                  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,isubfm,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,icomfm,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ihcfm,R,IOUT,in)   !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,isubun,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,icomun,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ihcun,R,IOUT,in)   !emrl ff
      endif  !emrl ff
