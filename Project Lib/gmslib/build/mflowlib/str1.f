     ...
     2                   LCTBAR,LCTRIB,LCIVAR,LCFGAR,ifrefm)
C                                                                      C
...
C2------ READ MXSTRM, NSS, NTRIB, ISTCB1, AND ISTCB2.                  C
  100 if (ifrefm.eq.0) then
    ...
    3   FORMAT(5I10,F10.0,2I10)
        ...
        read(in,'(a)') line                                     !emrl ff
        lloc=1                                                  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,mxstrm,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,nss,R,IOUT,in)     !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ntrib,R,IOUT,in)   !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ndiv,R,IOUT,in)    !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,icalc,R,IOUT,in)   !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,const,IOUT,in)   !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,istcb1,R,IOUT,in)     !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,istcb2,R,IOUT,in)   !emrl ff
      endif  !emrl ff
...
C10-----RETURN.                                                        C
     ...
     1                   NSS,NTRIB,IDIVAR,ICALC,IPTFLG,ifrefm) !emrl ff
C                                                                      C
C                                                                      C
...
C     -----------------------------------------------------------------C
     ...
     1          IDIVAR(NSS)
      integer*4 intlist(ntrib)        !emrl ff
      character*256  line      !emrl ff
C     -----------------------------------------------------------------C
C                                                                      C
...
C1B-----READ ITMP(NUMBER OF STREAM CELLS OR FLAG TO REUSE DATA).       C
      if (ifrefm.eq.0) then !emrl
        READ(IN,1)ITMP,IRDFLG,IPTFLG
    1   FORMAT(3I10)
      else  !emrl ff
        read(in,'(a)') line                                         !emrl ff
        lloc=1                                                      !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,itmp,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,irdflg,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,iptflg,R,IOUT,in)  !emrl ff
      endif !emrl ff
C                                                                      C
...
C5------READ AND PRINT DATA FOR EACH STREAM CELL.                      C
    ...
    5   FORMAT(5I5,F15.0,4F10.0)
        ...
        read(in,'(a)') line                                         !emrl ff
        lloc=1                                                      !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,K,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,I,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,J,R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ISTRM(4,II),R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,2,ISTRM(5,II),R,IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(1,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(2,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(3,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(4,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(5,II),IOUT,in)  !emrl ff
      endif !emrl ff
...
C6----READ AND PRINT DATA IF STREAM STAGE IS CALCULATED.               C
    ...
    8   FORMAT(3F10.0)
        ...
        read(in,'(a)') line                                         !emrl ff
        lloc=1                                                      !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(6,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(7,II),IOUT,in)  !emrl ff
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,STRM(8,II),IOUT,in)  !emrl ff
      endif !emrl ff
...
C9-----READ AND PRINT TRIBUTARY SEGMENTS.                              C
   ...
   11   FORMAT(10I5)
        ...
        read(in,*) intlist
        do jk=1,ntrib                  !emrl ff
          itrbar(ik,jk) = intlist(jk)  !emrl ff        
        enddo                          !emrl ff
      endif !emrl ff
