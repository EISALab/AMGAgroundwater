...
C2B-----READ PRIMARY STORAGE COEFFICIENT INTO ARRAY SC1 IF TRANSIENT.
      if(iss.eq.0)call SetLayRead(1,k,LayCon(k))                   !jd
...
C2C-----READ TRANSMISSIVITY INTO ARRAY CC IF LAYER TYPE IS 0 OR 2.
      ...
      call SetLayRead(2,k,LayCon(k))                               !jd
...
C2D-----IF LAYER TYPE IS 1 OR 3.
  100 CALL SetLayRead(4,k,LayCon(k))                               !jig KHFIX
c  100 CALL U2DREL(HY(1,1,KB),ANAME(3),NROW,NCOL,KK,IN,IOUT)       !jig KHFIX
      CALL U2DREL(HY(1,1,KB),ANAME(3),NROW,NCOL,KK,IN,IOUT)
      CALL SetLayRead(5,k,LayCon(k))                               !jig KHFIX
...
C2E-----MULTIPLIED BY CELL AREA TO CONVERT TO CONDUCTANCE LATER.
  110 IF(K.EQ.NLAY) GO TO 120
      call SetLayRead(3,k,LayCon(k))                               !jd
...
cjd      IF(THCK.LE.ZERO) GO TO 100                      !jd
         if(k.ne.nlay)then                               !jd
           if(thck.le.zero) go to 100                    !jd
         else                                            !jd
           if(NoDry.gt.0.0)then                          !jd
             if(thck.lt.NoDry)then                       !jd
               thck=NoDry                                !jd
             end if                                      !jd
           else                                          !jd
             if(thck.le.zero) go to 100                  !jd
