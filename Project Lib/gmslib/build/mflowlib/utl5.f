...
C
         ...
         call FluxObs(NCol,NRow,NLay,Text,Buff)              !jd
       end if                                                !jd

C3------RETURN
      RETURN
      END
      SUBROUTINE UCOLNO_MF(NLBL1,NLBL2,NSPACE,NCPL,NDIG,IOUT)
C
C
C-----VERSION 0934 22JUNE1992 UCOLNO_MF
C     ******************************************************************
...
C7------PRINT COLUMN NUMBERS ABOVE THE STRIP
      CALL UCOLNO_MF(J1,J2,ISP,NCAP,NCPF,IOUT)
C
...
C2------BETWEEN 1 AND 13.
    5 IP=IPRN
      IF(IP.LT.1 .OR. IP.GT.18) IP=12
C
C3------CALL THE UTILITY MODULE UCOLNO_MF TO PRINT COLUMN NUMBERS.
      IF(IP.EQ.1) CALL UCOLNO_MF(1,NCOL,0,11,11,IOUT)
      IF(IP.EQ.2) CALL UCOLNO_MF(1,NCOL,0,9,14,IOUT)
      IF(IP.GT.2 .AND. IP.LT.7) CALL UCOLNO_MF(1,NCOL,3,15,8,IOUT)
      IF(IP.GT.6 .AND. IP.LT.12) CALL UCOLNO_MF(1,NCOL,3,20,6,IOUT)
      IF(IP.EQ.12) CALL UCOLNO_MF(1,NCOL,0,10,12,IOUT)
      IF(IP.GE.13 .AND. IP.LE.18) CALL UCOLNO_MF(1,NCOL,3,10,7,IOUT)
C
...
401   CALL UCOLNO_MF(1,JJ,4,60,2,IOUT)
      GO TO 500
402   CALL UCOLNO_MF(1,JJ,4,40,3,IOUT)
      GO TO 500
403   CALL UCOLNO_MF(1,JJ,4,30,4,IOUT)
      GO TO 500
404   CALL UCOLNO_MF(1,JJ,4,25,5,IOUT)
      GO TO 500
405   CALL UCOLNO_MF(1,JJ,4,20,6,IOUT)
      GO TO 500
406   CALL UCOLNO_MF(1,JJ,4,10,12,IOUT)
      GO TO 500
407   CALL UCOLNO_MF(1,JJ,4,25,3,IOUT)
      GO TO 500
408   CALL UCOLNO_MF(1,JJ,4,15,5,IOUT)
      GO TO 500
409   CALL UCOLNO_MF(1,JJ,4,10,7,IOUT)
C
...
C6------IF PRINT CODE (IPRN) >0 OR =0 THEN PRINT ARRAY VALUES.
  320 continue                                                   !jd
      call ReReadLayer(a)                                        !jd
      IF(IPRN.GE.0) CALL ULAPRW(A,ANAME,0,0,JJ,II,0,IPRN,IOUT)   !jd
C
C7------RETURN
      RETURN
C
...
C-----EMRL COMMENT - INCLUDE THE QUOTE MARKS WHERE AVAILABLE
C        I=I+1
         IF(I.LE.LINLEN) THEN
            DO 25 J=I+1,LINLEN
            IF(LINE(J:J).EQ.'"') GO TO 39
25          CONTINUE
         END IF
C
...
C         RW(L:80)=LINE(ISTART:ISTOP)
         RW = LINE(ISTART:ISTOP)
         IF(NCODE.EQ.2) READ(RW,'(I80)',ERR=200) N
         IF(NCODE.EQ.3) READ(RW,'(F80.0)',ERR=200) R
      END IF
      RETURN
C
