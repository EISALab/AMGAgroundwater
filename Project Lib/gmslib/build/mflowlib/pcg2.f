...
c      READ(IN,2) MXITER,ITER1,NPCOND
c    2 FORMAT(3I)
C
C2------READ MAXIMUM NUMBER OF WELLS AND UNIT OR FLAG FOR
C2------CELL-BY-CELL FLOW TERMS.
C-------EMRL WE ALWAYS WILL WRITE IN FREE FORM SO IT IS ONLY SET UP TO HANDLE 
C-------FREE FORMAT-TJB
      READ(IN,'(A)') LINE
      LLOC=1
...
C      READ(IN,1) HCLOSE,RCLOSE,RELAX,NBPOL,IPRPCG,MUTPCG,DAMP
C*******EMRL JIG  this fix makes this function FREE FORM ONLY
      READ(IN,'(A)') LINE
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,HCLOSE,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,RCLOSE,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,RELAX,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NBPOL,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPRPCG,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MUTPCG,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,DAMP,IOUT,IN)
C    1 FORMAT(3F10.0,3I10,F10.0)
C
...
C--------CHANGED 510 FORMAT AND WRITE STATEMENT AND ADDED DEL 01MAY1993
         ...
         ENDIF
      ENDIF
510   FORMAT(' MATRIX IS SEVERELY NON-DIAGONALLY DOMINANT.  CHECK INPUT',
