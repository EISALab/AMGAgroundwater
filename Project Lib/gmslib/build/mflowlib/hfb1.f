...
C     ------------------------------------------------------------------
      ...
      character*256 line ! emrl 7/17/00 adding free format
      integer*4    col1,col2,row1,row2
      real         hydchr1
C
      COMMON/HFBCOM/NBRLAY(200)
...
C2D-----LOCATION AND HYDRAULIC CHARACTERISTIC.
      ...
      if (ifrefm.eq.0) then   !emrl ff
        READ(IN,5) row1,col1,row2,col2,hydchr1                   !emrl 7/17/00
    5   FORMAT(4I10,F10.0)                                       !emrl 7/17/00
      else  !emrl ff
        read(in,'(a)') line                                      !emrl 7/17/00
        lloc=1                                                   !emrl 7/17/00
        call URWORD(line,LLOC,ISTART,ISTOP,2,row1,R,IOUT,in)     !emrl 7/17/00
        call URWORD(line,LLOC,ISTART,ISTOP,2,col1,R,IOUT,in)     !emrl 7/17/00
        call URWORD(line,LLOC,ISTART,ISTOP,2,row2,R,IOUT,in)     !emrl 7/17/00
        call URWORD(line,LLOC,ISTART,ISTOP,2,col2,R,IOUT,in)     !emrl 7/17/00
        call URWORD(line,LLOC,ISTART,ISTOP,3,N,hydchr1,IOUT,in)  !emrl 7/17/00
      endif  !emrl ff

      WRITE(IOUT,6)ROW1,COL1,ROW2,COL2,HYDCHR1,II  ! emrl 7/17/00
    6 FORMAT(1X,19X,I5,3I10,5X,G12.4,I11)
      HFBR(1,II)=ROW1
      HFBR(2,II)=COL1
      HFBR(3,II)=ROW2
      HFBR(4,II)=COL2   ! emrl 7/17/00
      HFBR(5,II)=HYDCHR1 ! emrl 7/17/00
   90 CONTINUE
  100 CONTINUE
C
...
C3-----HALT EXECUTION IF ERRORS ARE DETECTED.
      IF (IERFLG.EQ.1)       call STOPFILE_MF
C
C4-----RETURN
      RETURN
      END
