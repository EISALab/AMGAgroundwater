...
/****************************************************************************
	Published version of ZRealMatrix 1.1
	...
	Update Records.
		1. UniRand() function is added.
		2. SetRandom(...) is finished.
*****************************************************************************/
#include <math.h>
#include <stdlib.h>
...
#define Verse Transpose
#define GetVerseMx GetTransposeMx
/*---------------------------------------------------------------------------------------
									const definition
----------------------------------------------------------------------------------------*/
...
inline REAL UniRand(){ return (REAL)(rand())/RAND_MAX; }
...
//matrix access
	ZRealMatrix GetTransposeMx() const;
	...
	//sum over the rows or cols.
	ZRealMatrix GetSumRowMx() const;
	ZRealMatrix GetSumColMx() const;
...
//matrix operator
	...
	ZRealMatrix& ElemAdd( const ZRealMatrix& mx );
	ZRealMatrix& ElemSub( const ZRealMatrix& mx );
	ZRealMatrix& ElemMul( const ZRealMatrix& mx );
	ZRealMatrix& ElemDiv( const ZRealMatrix& mx );
	ZRealMatrix& ElemExp();
	ZRealMatrix& ElemLog();
	REAL DotMul( const ZRealMatrix& mx );
