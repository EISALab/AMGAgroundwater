...
integer, parameter :: MAX_SAMPLES = 60
...
implicit none
	...
	do i=1, sgaParam%nPopSize
		if( SearchChromCache(hCacheTemp, sgaParam%arrPop(i,:), sgaParam%nChromBits, 0, dums, 1)==0 )then
			...
			!only sample solutions with samples smaller than MAX_SAMPLES
			nSamples = GetSampleCountCache(hChromCache, sgaParam%arrPop(i,:), sgaParam%nChromBits )
			if( UniRand()>dble(nSamples)/MAX_SAMPLES )then
!			if( nSamples < MAX_SAMPLES )then
				count = count+1
				arrIds(count) = i
			endif
		endif
...
subroutine BestSelPdes( sgaParam, nExpectedPdes, arrPdeIds, nPdeCount )
use std_helper
implicit none
	type(SGAPARAMSTRUCT), intent(in) :: sgaParam
!	integer, dimension(:) :: arrIndex
	...
	call UniqueIds( sgaParam, arrIndex, nSelCount )
	call qsort( arrIndex, nSelCount, sizeof(arrIndex(1)), CompareByCostViolTest )
...
implicit none
	...
	double precision :: rViolRdx, rViolTnt, rViolWell, rReliab
	...
	!call neural network
	call predict_meta_model( hFourYCModel, arrVars, arrVars(nInputs+1:nInputs+2) )
	call predict_meta_model( hFiveYCModel, arrVars, arrVars(nInputs+3:nInputs+4) )
	!parse the data back to global variables.
	call UnNormalize( hNormMean, arrVars, 1 )

	!the first model predicts reliability and VCG
	rReliab = arrVars(nInputs+1)
	if( rReliab<0 )rReliab = 0
	if( rReliab>1 )rReliab = 1.0
	rDummyVCGCost = 10**arrVars(nInputs+2)

	!the second model predicts RDX and TNT
	rMaxRdxC = 10**arrVars(nInputs+3)
	rMaxTntC = 10**arrVars(nInputs+4)
	...
	sgaParam%arrReliabs(index) = rReliab
...
!	rFitness = rFitness + MB1 + MB2
	rFitness = rTotalCost + MB1 + MB2 + (rViolTnt+rViolRdx)*2000.0
!	rFitness = rTotalCost + sgaParam%arrViols(index) * 50

!	rFitness = log(rFitness + MB1 + MB2)

	!the cached array is (rTotalCost, rViols, rFitness, rMaxRdxC, rMaxTntC, rFourMaxRdxC, rFourMaxTntC, rVCGCost, nRemYear, rReliab )
...
!	if( present(arrCachedStds) )arrCachedStds = (/rDummyVCGStdCost, sgaParam%arrViolStds(index), -1.d0, rDummyVCGStdCost, rMaxRdxStdC, rMaxTntStdC, arrVars(nInputs+1), arrVars(nInputs+2), rDummyVCGStdCost, 0.d0/)
	...
	ComputeModelFitness = rFitness + ComputeReliabPen( rReliab )
end function
...
implicit none
	...
	do i=lbound(sgaParam%arrFitInj, 1), ubound(sgaParam%arrFitInj,1)
		...
		if( SearchChromCache(hChromCache, sgaParam%arrPopInj(i,:), sgaParam%nChromBits, -1, arrCachedFits, nCachedFits)/=0 )then
			...
			sgaParam%arrFitInj(i) = arrCachedFits(3) + ComputeReliabPen( arrCachedFits(10) )
			sgaParam%arrFitStdsInj(i) = arrCachedFitStds(3)
			sgaParam%arrReliabInj(i) = arrCachedFits(10)
		endif
	enddo
end subroutine
...
implicit none
	...
	do i = 1, nIds
		...
		if( SearchChromCache(hChromCache, sgaParam%arrPop(id,:), sgaParam%nChromBits, -1, arrCachedFits, nCachedFits)/=0 )then
			...
			sgaParam%arrFit(id) = arrCachedFits(3) + ComputeReliabPen( arrCachedFits(10) )
!			sgaParam%arrFit(id) = arrCachedFits(3)
			sgaParam%arrFitStds(id) = arrCachedFitStds(3)
			!save the reliabilities
			sgaParam%arrReliabs(id) = arrCachedFits(10)
...
implicit none
	...
	integer :: iDryCellC, iDryCellH, nRemYear
	double precision :: rFourMaxRdxC, rFourMaxTntC, rReliab
	...
	do i=1, nJobCount
		...
		if( rMaxRdxC>900 .or. rMaxTntC>900 )then
			...
			!Mote, the fitness will be evaluated by the cache.
			rFitness = rTotalCost + MB1 + MB2 + (rViolRdx+rViolTnt)*2000.0
!			rFitness = rTotalCost + rViols*50

!			rFitness = rFitness + MB1 + MB2
!			rFitness = log(rFitness + MB1 + MB2)

			if( rViolRdx+rViolTnt<=1e-6 )then
				rReliab = 1.0
			else
				rReliab = 0.0
			endif
			...
			!the cached array is (rTotalCost, rViols, rFitness, rMaxRdxC, rMaxTntC, rFourMaxRdxC, rFourMaxTntC, rVCGCost, nRemYear, rReliab )
			arrCachedFits = (/rTotalCost, rViols, rFitness, rMaxRdxC, rMaxTntC, rFourMaxRdxC, rFourMaxTntC, rDummyVCGCost, dble(nRemYear), rReliab/)
...
contains
	...
	implicit none
		...
		double precision, dimension(nCachedFits) :: arrCachedFits, arrCachedFitStds
		double precision :: rMaxRdxC, rMaxTntC, rFourMaxRdxC, rFourMaxTntC, rDummyVCGCost, rReliab
...
!		arrVars(nInputs+2) = rMaxTntC / rFourMaxTntC
		...
		!the first model predicts reliability and VCG
		arrVars(nInputs+1) = rReliab
		arrVars(nInputs+2) = log10(rDummyVCGCost)
		!the second model predicts RDX and TNT
		arrVars(nInputs+3) = log10(rFourMaxRdxC)
		arrVars(nInputs+4) = log10(rFourMaxTntC)

		call Normalize( hMeanNorm, arrVars, 1 )

		!writting it to the training file or testing file
		call AppendToFile( strFourYCTrain, arrVars, nInputs+2 )
		call AppendToFile( strFiveYCTrain, (/arrVars(1:nInputs), arrVars(nInputs+3:nInputs+4)/), nInputs+2 )
...
implicit none
	...
	double precision, dimension(nCachedFits) :: arrCachedFits, arrCachedFitStds
	double precision :: MB1, MB2, rWellViol, rReliab
...
!			arFit = (/ arrCachedFits(3), arrCachedFitStds(3) /)
			arFit = (/ sgaParam%arrFit(id), arrCachedFitStds(3) /)
			...
			rReliab = sgaParam%arrReliabs(id)
		...
		call SaveIndInfo( id, sgaParam%arrFlags(id), samples, rReliab, arFit, arCost, arViol, arRdx5, arTnt5, arVCG, arCoeff)
...
!			& g_arrWellLocs, g_arrPumpRates, arrHeads, arrHeadStds, sgaParam%arrPop(i,:) )
	...
	1000 format( 3('#'), 'Generation ', i4, 2x, 3('#'), 'reliability fitness cost violations, totcost, rdx5, tnt5, VCG, loc[], Q[], H[]', 10('#') )

contains

	subroutine SaveIndInfo( id, nFlag, samples, rReliab, arFit, arCost, arViol, arRdx5, arTnt5, arVCG, arCoeff )
	implicit none

	!arguments
	integer, intent(in) :: id, nFlag, samples
	double precision :: rReliab
	double precision, dimension(2) :: arFit, arCost, arViol, arRdx5, arTnt5, arVCG, arCoeff
	...
	write(hGaOut,fmt=1100,advance="NO")rReliab, arFit, arCost, arViol, arRdx5, arTnt5, arVCG, arCoeff

	!well locations
	do j=1, g_nNewWells
		write( hGaOut, fmt=1080, advance="NO" )nint(sgaParam%arrVars(id, j))
	enddo
	!well pumping rates
	...
	1100 format( 2x, f6.4, (f12.4, '(', f12.4, ')'), 2(f12.4, '(', f10.6, ')'), 4(f12.8, '(', f12.8, ')') )
	1200 format( 2x, i2)
	1300 format(2x, f9.4)
...
implicit none                                                                                                                                                                                             
	...
	!the cached array is (rTotalCost, rViols, rFitness, rMaxRdxC, rMaxTntC, rFourMaxRdxC, rFourMaxTntC, rVCGCost, nRemYear, rReliab )
	nCachedFits = 10		!(cost, violations, fitnes, rdx(5), tnt(5), rdx(4), tnt(4), VCG, nRemYear, rReliab )
...
!			call BestSelPdes( sgaParam, arrIndex, nExpectedPdes, arrPdeIds, nPdeIds)
			call BestSelPdes( sgaParam, nExpectedPdes, arrPdeIds, nPdeIds)
		else
			call TournSelPdes( sgaParam, 8, nExpectedPdes, arrPdeIds, nPdeIds )
		endif
	else
		call UniqueIds( sgaParam, arrPdeIds, nPdeIds )

!		nPdeIds = 0
!		do i = 1, sgaParam%nPopSize
			!this is a noisy GA, cached solutions should still be sampled.
...
!		enddo
	...
	!choose the realizations using non-replacement incremental latin tube
 	call ChooseRealizations2( sgaParam, arrPdeIds, nPdeIds, sgaParam%arrRealizations )
