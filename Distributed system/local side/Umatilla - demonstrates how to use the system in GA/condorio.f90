module condorio
use ncpfor
use stdfor
use std_helper
implicit none

integer, parameter :: hSurrBack	= 1500
integer, parameter :: hSurrGo	= 1501

character(*), parameter :: strSurrGo	= 'surrgo.dat'
character(*), parameter :: strSurrBack	= 'surrbak.dat'
character(*), parameter :: strFileList	= '../sendfiles.dat'

character(*), parameter :: strSlave		= 'slave'
character(*), parameter :: strStatic	= 'static'
character(*), parameter :: strStopFile	= 'grid/ga.stop'
character(*), parameter :: strRestart	= 'grid/ga.restart'

logical, parameter :: bLocalRun = .false.
!logical, parameter :: bLocalRun = .true.

contains

subroutine MakeSlaveName( str, nId, strRet )
implicit none
	character(*), intent(in) :: str
	integer, intent(in) :: nId
	character(*), intent(out) :: strRet

	!variables
	character * 80 :: buf
	call int_to_str( nId, buf )
	call str_cat( str, buf, strRet )
end subroutine

subroutine MakeStaticFolder
implicit none
	call RemoveDir( strStatic )
	call MakeDir( strStatic )
end subroutine MakeStaticFolder

subroutine PreSlave( nId, nGen )
implicit none
	integer, intent(in) :: nId, nGen

	!variable
	character(80) :: strSlaveFolder

	call MakeSlaveName( strSlave, nId, strSlaveFolder )
	call MakeDir( strSlaveFolder )
	call ChangeDir( strSlaveFolder )

	if( nGen/=-1 )call CopySlaveFiles( strFileList )

end subroutine PreSlave

subroutine WaitForSlaves(arrJobIds, nCount)
implicit none
	!argument
	integer, dimension(*), intent(in) :: arrJobIds
	integer, intent(in) :: nCount

	!variables
	integer :: ret
	logical :: stopflag
	integer :: i

	!clean stop file flag
	ret = RemoveFileC( strStopFile )

	if( bLocalRun )then
		do i=1, nCount
			call PreSlave(arrJobIds(i), -1)
			call WaiteForExec( 'surrga.exe' )
			call PostSlave
		end do
	else
		call RouteSlaves( arrJobIds, nCount )
		!call WaiteForExec( 'grid/control' )
	endif

	stopflag = .false.
	inquire( FILE=strStopFile, EXIST=stopflag )
	if( stopflag )then
		print*, 'control signals ga to stop, stoppping!'
		stop;
	endif
end subroutine

subroutine PostSlave
implicit none
	
	call ChangeDir( '..' )
end subroutine PostSlave

subroutine RemoveSlave( nId )
implicit none
	integer, intent(in) :: nId
	!variable
	character(80) :: strSlaveFolder

	call MakeSlaveName( strSlave, nId, strSlaveFolder )
	call RemoveDir( strSlaveFolder )
end subroutine

subroutine BroadcastSlave(individual, generation, gridflagx, gridflagy )
implicit none
	!arugments
	integer, intent(in) :: individual, generation
	integer, intent(in) :: gridflagx, gridflagy

	!variables
	integer :: ret, i

	call PreSlave( individual, int(generation) )

	call OpenOutputFile( hSurrGo, strSurrGo )
	rewind(hSurrGo)

	write( hSurrGo, * )individual, int(generation)
	write( hSurrGo, * )gridflagx, gridflagy

	close( hSurrGo )
	
	!add 5/22/03 to handle crashed salves
	ret = RemoveFileC( strSurrBack )

	call PostSlave
end subroutine BroadcastSlave

!CollectSlaveEx returns violations of Rdx and Tnt.
subroutine CollectSlaveEx( individual, generation, rFitness, rTotCost, rViolRdx, rViolTnt, c_max_rdx, c_max_tnt, rDummyVCFCost, nRemYear, rFourMaxRdxC, rFourMaxTntC, drycell_c, drycell_h )
use costfun
implicit none
	!arguments
	integer, intent(in) :: individual, generation
	double precision, intent(out) :: rFitness
	double precision, intent(out) :: rTotCost, rViolRdx, rViolTnt, c_max_rdx, c_max_tnt, rDummyVCFCost
	integer, intent(out) :: drycell_c, drycell_h, nRemYear
	double precision, intent(out) :: rFourMaxRdxC, rFourMaxTntC

	!variables,
	double precision :: Pen_Rdx, Pen_Tnt

	call CollectSlave( individual, generation, rFitness, rTotCost, Pen_Rdx,Pen_Tnt, c_max_rdx, c_max_tnt, rDummyVCFCost, nRemYear, rFourMaxRdxC, rFourMaxTntC, drycell_c, drycell_h )

	call ComputeRdxTntViols( Pen_Rdx, Pen_Tnt, rViolRdx, rViolTnt )

	call RemoveSlave( individual )
end subroutine


subroutine CollectSlave( individual, generation, rFitness, rTotCost, Pen_Rdx,Pen_Tnt, c_max_rdx, c_max_tnt, rDummyVCFCost, nRemYear, rFourMaxRdxC, rFourMaxTntC, drycell_c, drycell_h )
implicit none
	!argument
	integer, intent(in) :: individual, generation
	double precision, intent(out) :: rFitness
	double precision, intent(out) :: rTotCost, Pen_Rdx, Pen_Tnt, c_max_rdx, c_max_tnt, rDummyVCFCost
	integer, intent(out) :: drycell_c, drycell_h, nRemYear
	double precision, intent(out) :: rFourMaxRdxC, rFourMaxTntC

	!variable
	logical :: backexist
	integer :: indiv_read, gen_read, i

	call PreSlave( individual, -1 )

	!add 5/22/03 to handl crashed slave
	inquire( FILE=strSurrBack, EXIST=backexist )
	if( backexist )then
		call OpenInputFile( hSurrBack, strSurrBack )
		rewind( hSurrBack )

		read( hSurrBack, * )indiv_read, gen_read
		read( hSurrBack, * )rFitness, rTotCost
		read( hSurrBack, * )Pen_Rdx, Pen_Tnt
		read( hSurrBack, * )c_max_rdx, c_max_tnt
		read( hSurrBack, * )drycell_c, drycell_h
		read( hSurrBack, * )rDummyVCFCost, nRemYear
		read( hSurrBack, * )rFourMaxRdxC, rFourMaxTntC

		close( hSurrBack )
	else
		print*, 'warning: surrback file is not found!', individual
		!stop
	endif

	call PostSlave
end subroutine CollectSlave

end module condorio
