...
type SGAPARAMSTRUCT
	...
	double precision, allocatable, dimension(:) :: arrReliabInj
	...
	logical :: bSeedingBits, bSeedingVars
	character(40) :: flSeeding
...
subroutine LoadSeedings( filename, sgaParam )
implicit none
	type(SGAPARAMSTRUCT), intent(inout) :: sgaParam
	character(*) :: filename

	integer :: hFile, i, id, count

	count = count_nonblank_lines( filename )

	hFile = 1000
	call OpenInputFile( hFile, filename )
	do i=1, count
		id = ceiling(UniRand()*sgaParam%nPopSize)
		if( id>sgaParam%nPopSize )id = sgaParam%nPopSize
		read( hFile, * )sgaParam%arrPop(id,:)
	enddo
	close( hFile )

end subroutine

subroutine LoadSeedingVars( filename, sgaParam )
implicit none
	type(SGAPARAMSTRUCT), intent(inout) :: sgaParam
	character(*) :: filename
	...
	do i=1, count
		id = ceiling(UniRand()*sgaParam%nPopSize)
		read( hFile, * )sgaParam%arrVars(id,:)
		call EncodeOne( sgaParam, id )
	enddo
	close( hFile )
...
implicit none
	...
	double precision, dimension( sgaParam%nPopSize*2 ) :: arrFitPool, arrFitStdPool, arrCostPool, arrCostStdPool, arrViolPool, arrViolStdPool, arrReliabPool
	...
	if( sgaParam%bSeedingBits )call LoadSeedings( sgaParam%flSeeding, sgaParam )
	if( sgaParam%bSeedingVars )call LoadSeedingVars( sgaParam%flSeeding, sgaParam )
...
!	arrFlagsPool = NN_EVAL
	...
	do iCur = iStart, iEnd
		...
		do i=1, sgaParam%nPopSize
			...
			arrReliabPool( sgaParam%nPopSize+i ) = sgaParam%arrReliabs(i)

			arrFlagsPool(sgaParam%nPopSize+i) = sgaParam%arrFlags(i)
			arrChromPool(sgaParam%nPopSize+i, :) = sgaParam%arrPop(i,:)
		enddo

		!update the PDE results in the parent, the extended sampling may affect earlier solutions
		arrIndex = (/(i, i=1,sgaParam%nPopSize*2)/)
		if( sgaParam%iCurGen>1 )then
			!the random bits will cause SearchChromCache assertion error.
			call UpdateByCache( arrIndex, sgaParam%nPopSize, 10, CACHE_EVAL )
		endif
...
!		endif
		...
		do i = 1, sgaParam%nPopSize
			...
			sgaParam%arrReliabs(i) = arrReliabPool( arrIndex(i) )
...
!			endif
		...
		arrReliabPool(1:sgaParam%nPopSize) = sgaParam%arrReliabs
...
!	if( bConverged )then
	...
	do i=1, sgaParam%nPopSize
		call DumpChrom( hChromCache, sgaParam%arrPop(i,:), sgaParam%nChromBits )
	enddo
	i = RemoveFileC( strRestartFile )
...
subroutine UpdateByCache( arrIds, nIds, nCachedFits, nFlag )
use chmcache
use costfun
implicit none
	!argument                                                                                                                                                                                         
	...
	do i = 1, nIds
		id = arrIds(i)
		if( SearchChromCache(hChromCache, arrChromPool(id,:), sgaParam%nChromBits, -1, arrCachedFits, nCachedFits)/=0 )then
			call GetStatChromCache(hChromCache, arrChromPool(id,:), sgaParam%nChromBits, arrCachedFits, arrCachedFitStds )
			arrCostPool(id) = arrCachedFits(1)
			arrViolPool(id) = arrCachedFits(2)
			arrCostStdPool(id) = arrCachedFitStds(1)
			arrViolStdPool(id) = arrCachedFitStds(2)
			arrFitPool(id) = arrCachedFits(3) + ComputeReliabPen( arrCachedFits(10) )
			arrFitStdPool(id) = arrCachedFitStds(3)

			!save the reliabilities
			arrReliabPool(id) = arrCachedFits(10)
			arrFlagsPool(id) = nFlag
		endif
	enddo
end subroutine
...
implicit none
	...
	if( sgaParam%iCurGen>1 )then
		do i=lbound(sgaParam%arrFitInj, 1), ubound(sgaParam%arrFitInj,1)
			...
			sgaParam%arrReliabs(id) = sgaParam%arrReliabInj(i)
	...
	do i=lbound(sgaParam%arrFitInj, 1), ubound(sgaParam%arrFitInj,1)
		...
		do k=lbound(sgaParam%arrFitInj, 1), ubound(sgaParam%arrFitInj,1)
			if( sgaParam%arrFit(id)<sgaParam%arrFitInj(k) )then
				...
				sgaParam%arrReliabInj(k) = sgaParam%arrReliabs(id)
				exit
			endif
		enddo
	enddo
...
implicit none
	...
	do id = 1,sgaParam%nPopSize
		...
		if( sgaParam%arrFlags(id)==NN_EVAL )then		!neural network
			write( hFile, fmt=1001, advance="NO" ) id, samples, sgaParam%arrReliabs(id), sgaParam%arrFit(id)
		else if( sgaParam%arrFlags(id)==CACHE_EVAL )then	!from cache
			write( hFile, fmt=1002, advance="NO" ) id, samples, sgaParam%arrReliabs(id), sgaParam%arrFit(id)
		else if( sgaParam%arrFlags(id)==PDE_EVAL )then
			write( hFile, fmt=1003, advance="NO" ) id, samples, sgaParam%arrReliabs(id), sgaParam%arrFit(id)
		else
			call assert( .false. )
		endif
	...
	1001 format( i4, '(A-', i3, ')', 2x, f6.4, 2x, f17.4 )
	1002 format( i4, '(C-', i3, ')', 2x, f6.4, 2x, f17.4 )
	1003 format( i4, '(P-', i3, ')', 2x, f6.4, 2x, f17.4 )
