...
{
	//tries to bind a local address, let system to select the port number
	SOCKADDR_IN clntaddr;
	clntaddr.sin_family = AF_INET;
	clntaddr.sin_addr.s_addr = INADDR_ANY;
	clntaddr.sin_port = htons( 0 );
	Bind( sock, (SOCKADDR*)&clntaddr, sizeof(SOCKADDR_IN) );

	for( int i=0; i<nRetry; i++ ){
		bool bBlockedPort = false;
		//check the bound port
		int len = sizeof(SOCKADDR_IN);
		GetSockName( sock, (SOCKADDR*)&clntaddr, &len );
		//the following ports are probably blocked by firewalls, they can cause delays (timed-out)
		switch( ntohs(clntaddr.sin_port) ){
		case 3127:
		case 1434:
		case 2745:
			bBlockedPort = true;
		...
		//try to bind to a new local port
		if( bBlockedPort ){
			cdump<<lock<<"BLOCKED port is bound "<<i+1<<" times on local port "<<ntohs(clntaddr.sin_port)<<endl<<unlock;
			//close the socket and create a new socket, hopelly I can bind to a different port
			::closesocket( sock );
	...
	return ::Connect( sock, (SOCKADDR*)&addr, sizeof(SOCKADDR_IN) );
//	return ::connect( sock, (SOCKADDR*)&addr, sizeof(SOCKADDR_IN) );

/*	//tries to bind a local address, let system to select the port number
	//this may slightly increase the chances of binding to good ports.
	SOCKADDR_IN clntaddr;
	clntaddr.sin_family = AF_INET;
	clntaddr.sin_addr.s_addr = INADDR_ANY;
	clntaddr.sin_port = htons( 0 );
	...
	for( int i=0; i<nRetry; i++ ){
		...
		int len = sizeof(SOCKADDR_IN);
		GetSockName( sock, (SOCKADDR*)&clntaddr, &len );
		cdump<<lock<<"redundant connections failed "<<i+1<<" times on local port "<<ntohs(clntaddr.sin_port)<<endl<<unlock;

		//this socket is probably bind to a blocked port number. 
...
/*		CSocket sk;
		...
		bool bOk;
		sk.Attach( sock );
		bOk = sk.GetSockName( addrIn );
		int nPort = addrIn.GetPort();

		int yes = 0;
		int len = sizeof(len);
		bOk = sk.GetSockOpt( SO_REUSEADDR, (char*)&yes, &len );
		sk.Close();*/
...
/*		sock = Socket( PF_INET, SOCK_STREAM, 0 );
		...
		Bind( sock, (SOCKADDR*)&srvaddr, sizeof(SOCKADDR_IN) );

	}
	return nRet;*/
}

#define ntohll(x) (((__int64)(ntohl((int)((x << 32) >> 32))) << 32) | (unsigned int)ntohl(((int)(x >> 32)))) //By Runner
