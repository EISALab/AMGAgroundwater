...
public:
	...
	CMessageHead* GetMessage( int nMessage ){
	...
	HRESULT DoRmDir( const string& strPath, bool bRecursive );
	HRESULT DoPassive( u_int& nPasPort );
	HRESULT DoPutFile( const string& strFileName, bool bOverwrite );
	HRESULT DoGetFile( const string& strFileName );
...
public:
	CClientProxy( const CSessionAddr& addr, IN_ADDR addrServ ):m_addr(addr), m_addrServ(addrServ){
	...
	void Shutdown();
	void Broadcast( CMessageHead* pMsg ){
		CObjLocker<CClientProxy> locker(this);
...
public:
	...
	void Unlock(){ UnlockMutex(m_hMutex); }
protected:
	CClientProxy* GetClntProxy( IN_ADDR addr ){
