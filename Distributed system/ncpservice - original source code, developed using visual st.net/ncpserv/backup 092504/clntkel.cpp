...
bool CClientSession::Open()
{
	if( m_pClntProxy->OpenChannel(this) ){
		...
		msg.hton();
		m_pClntProxy->SendMessage( msg.GetMessageHead() );

		CMessageHead* pAck = GetMessage(NCF_ACK|NCM_OPENSESSION);
...
{
	CMessageHead* pMsg = (CMessageHead*)new char[sizeof(CMessageHead)+strUser.length()+strPassword.length()+2];
	pMsg->Init( NCM_LOGON, GetAddr(), GetServAddr() );
	strcpy( pMsg->GetData(), strUser.c_str() );
	strcpy( pMsg->GetData()+strUser.length()+1, strPassword.c_str() );
	pMsg->SetLength( sizeof(CMessageHead)+strUser.length()+strPassword.length()+2 );

	CMessageTrash trash(pMsg);
	m_pClntProxy->SendMessage( pMsg );

	CMessageHead* pAck = GetMessage( NCF_ACK|NCM_LOGON );
...
HRESULT CClientSession::DoMkDir( const string& strPath )
{
	...
	pMsg->Init( NCM_MKDIR, GetAddr(), GetServAddr() );
	...
	CMessageHead* pAck = GetMessage( NCF_ACK|NCM_MKDIR );
	CMessageTrash trash2( pAck );
	return pAck->GetResult();
}
...
{
	CMessageHead* pMsg = (CMessageHead*)new char[sizeof(CMessageHead)+strPath.length()+1];
	pMsg->Init( NCM_CHDIR, GetAddr(), GetServAddr() );
	pMsg->SetLength( sizeof(CMessageHead)+strPath.length()+1 );
	strcpy( pMsg->GetData(), strPath.c_str() );
	...
	CMessageHead* pAck = GetMessage( NCF_ACK|NCM_CHDIR );
	CMessageTrash trash2( pAck );
	return pAck->GetResult();
}
	

HRESULT CClientSession::DoRmDir( const string& strPath, bool bRecursive )
{
	CMessageHead* pMsg = (CMessageHead*)new char[sizeof(CMessageHead)+strPath.length()+1];
	pMsg->Init( NCM_RMDIR, GetAddr(), GetServAddr(), bRecursive );
	pMsg->SetLength( sizeof(CMessageHead)+strPath.length()+1 );
	strcpy( pMsg->GetData(), strPath.c_str() );
	...
	CMessageHead* pAck = GetMessage( NCF_ACK|NCM_RMDIR );
	CMessageTrash trash2( pAck );
	return pAck->GetResult();
}
...
HRESULT CClientSession::DoPassive( u_int& nPasPort )
{
	...
	CMessage1Param<u_int>* pMsg = (CMessage1Param<u_int>*)GetMessage( NCF_ACK|NCM_PASSIVE );
	CMessageTrash trash( pMsg->GetMessageHead() );
	if( pMsg->GetMessageHead()->IsSucceeded() ){
		pMsg->ntoh();
		nPasPort = pMsg->GetParam();
	}
	return pMsg->GetMessageHead()->GetResult();
}
...
{
	CMessageHead* pMsg = (CMessageHead*)new char[sizeof(CMessageHead)+strCommand.length()+1];
	pMsg->Init( NCM_RUNIMAGE, GetAddr(), GetServAddr(), bSync );
	pMsg->SetLength( sizeof(CMessageHead)+strCommand.length()+1 );
	strcpy( pMsg->GetData(), strCommand.c_str() );

	CMessageTrash trash(pMsg);
	m_pClntProxy->SendMessage( pMsg );

	CMessage1Param<int>* pAck = (CMessage1Param<int>*)GetMessage( NCF_ACK|NCM_RUNIMAGE );
	CMessageTrash trash2( pAck->GetMessageHead() );
	if( pAck->GetMessageHead()->IsSucceeded() ){
		pAck->ntoh();
		dwExitCode = pAck->GetParam();
	}
	return pAck->GetMessageHead()->GetResult();
}
...
{
	CMessageHead msg;
	msg.Init( NCM_FILEINFO, GetAddr(), GetServAddr() );
	m_pClntProxy->SendMessage( &msg );

	CMessage1Param<NETF_STAT>* pAck = (CMessage1Param<NETF_STAT>*)GetMessage( NCF_ACK|NCM_FILEINFO );
	CMessageTrash trash( pAck->GetMessageHead() );
	if( pAck->GetMessageHead()->IsFailed() ){
		return pAck->GetMessageHead()->GetResult();
	}
	fstat = pAck->GetParam();
	return pAck->GetMessageHead()->GetResult();
...
/********************************************************************************************
										CClientProxy
*********************************************************************************************/
bool CClientProxy::SendMessage( CMessageHead* pMsg )
{
...
int CClientProxy::ClientProxyThread( void* arg )
{
	...
	while( true ){
		CMessageHead* pMsg = RecvMessage( pProxy->m_sock );
		if( pMsg==NULL ){	//sock shutdown.
			CMessageHead msg(NCM_PROXYSHUTDOWN);
			pProxy->Broadcast( &msg );
			break;
		}
		CClientSession* pSession = pProxy->GetSession( pMsg->GetDstAddr() );
		if( pSession )pSession->PostMessage( pMsg );
	}
	return 0;
}
...
//********************************************************************************************************
void CClientManager::AddHost( char* strHost )
{
...
void CClientManager::OnAckPoll( CMessageHead* pMsg ){
	CObjLocker<CClientManager> locker(this);
