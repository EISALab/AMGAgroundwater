...
//	void ReturnServerSession( CServerSession* pServSession );
	...
	IN_ADDR GetInAddr(){ return m_addr.m_addrHost; }
	...
	//forward the shutdown message to the true servers. (close the whole system)
	void DoForwardShutdown( SOCKET sock );

	//stream connection message
	bool MessageFromStream( SOCKET sock );

	void Initialize();
	void RunServer();

//	static int ServiceThread( void* arg );
...
public:
	...
	u_short GetPassivePort(){ return m_pServApp->GetPassivePort(); }
