...
#ifdef _HOOK_FIREWALL
		//this is to hook up tunneled messages, since the destinations of which are LOOPBACK.
		CSessionAddr addrDst = pMsg->GetDstAddr();
		if( !(addrDst.m_addrHost== GetAddr().m_addrHost) ){
			addrDst.m_addrHost = GetAddr().m_addrHost;
		}
		pMsg->SetDstAddr( addrDst );
#endif
	...
	case NCM_OPENSESSION:
		{
			...
			pOpenMsg->ntoh( false );
			//this is to hookup tunneled messages, since the address is LOOPBACK
			CSessionAddr addrServ = pOpenMsg->GetParam();

#ifdef _HOOK_FIREWALL
			if( !(addrServ.m_addrHost==GetAddr().m_addrHost) ){
				addrServ.m_addrHost = GetAddr().m_addrHost;
			}
#endif

			OnOpenSession( pMsg->GetSrcAddr(), addrServ );
...
{
	//suspended service doesn't accept more sessions.
...
void CServerProxy::WaitForObject()
{
	HANDLE hProxyThread = NULL;
	{
		...
		hProxyThread = m_hProxyThread;
		m_hProxyThread = NULL;
	}

	WaitForThread( hProxyThread, NULL );
	//the thread will destroy proxy object when exiting
...
void CServerSessionStub::OpenStream( SOCKET sock )
{
	...
	if( hThread==NULL ){
		//can't create the stream proxy thread...
		closesocket( sock );
		delete pParam;
		return;
	}
...
//	cdump<<"delete proxy thread param:"<<(void*)pParam<<endl;
	...
	try{
		...
		bool socks_ok[2];
		socks_ok[0] = socks_ok[1] = true;

		while( socks_ok[0] || socks_ok[1] ){
			FD_ZERO( &rfds );
			maxfds = 0;
			if( socks_ok[0] ){
				FD_SET( sock_cltdata, &rfds );
				maxfds = max( sock_cltdata, maxfds );
			}
			if( socks_ok[1] ){
				FD_SET( sock_srvdata, &rfds );
				maxfds = max( sock_srvdata, maxfds );
			}
//			maxfds = max(sock_cltdata,sock_srvdata)+1;
			...
			if( FD_ISSET(sock_cltdata, &rfds) ){
				...
				if( nLen<=0 ){
					//no data coming from client again, shutdown the server's send
					shutdown( sock_srvdata, SD_SEND );
					socks_ok[0] = false;
					if( nLen<0 )break;
				}else{
					clnt_data += nLen;
			...
			if( FD_ISSET(sock_srvdata, &rfds) ){
				...
				if( nLen<=0 ){
					//no data coming from server again, shutdown the clients' send
					shutdown( sock_cltdata, SD_SEND);
					socks_ok[1] = false;
					if( nLen<0 )break;
				}else{
					serv_data += nLen;
	...
	cdump<<lock<<"Client:"<<clnt_data<<", Server:"<<serv_data<<endl<<unlock;
	return 0;
}
...
{
	while( true ){
//		cdump<<lock<<"("<<GetAddr().m_nServId<<")Waiting for message "<<nMessage<<endl<<unlock;
		CMessageBase* pMsg = m_MsgQueue.GetMessage();
//		cdump<<lock<<"("<<GetAddr().m_nServId<<")Message:"<<"Ack("<<pMsg->IsAcking()<<") "<<" OOB("<<isset(pMsg->GetMessage(),NCF_OOB)<<") "<<pMsg->GetCommand()<<endl<<unlock;
...
void CServerSessionLocal::AckRunImage( const string& strCommand, bool bSync )
{
	...
	if( bSync ){	//synchronize running, register the handle and wait the process
		...
		if( hMonitorThread==NULL ){
			if( param.hSema!=NULL )CloseHandle( param.hSema );
			AckMessage( NCM_RUNIMAGE, E_THREAD );
			return;
		}
...
#ifndef _WIN32
	if( m_hProxyThread!=NULL )
		pthread_kill( HANDLE_TO_THREADID(m_hProxyThread), SIGUSR1 );
#endif
}
...
//	cdump<<"delete proxy:"<<(void*)pServProxy<<endl;
	{
		CObjLocker<CServerProxy> locker(pServProxy);
		pServProxy->ReleaseProxy();
	}
	delete pServProxy;
}

void CServerApp::Destroy()
{
...
void CServerApp::Initialize()
{
	m_addr.m_addrHost = GetLocalInAddr();
	m_addr.m_nServId = 0;

	//setup the cpu tokens
	if( IsAsServer() ){
		CPU_INFO cpuinfo;
        GetCpuInfo( &cpuinfo );
...
{
	Initialize();
	//m_hServiceThread = CreateThread( (THREAD_ROUTINE)ServiceThread, this, 0 );

	//setup the session accepting socket.
	CSocket sockacpt;
	sockacpt.Create( NCP_SERVER_PORT );
	sockacpt.Listen();
...
//		msg.Broadcast( sockbroad, NCP_SERVER_PING_PORT );
	...
	//note. SetSockOpt SO_REUSEADDR must be called before bind. Otherwise it won't work.
	int yes = 1;
	sockpas.SetSockOpt( SO_REUSEADDR, (char*)&yes, sizeof(yes) );

#ifdef _HOOK_FIREWALL
	sockpas.Bind( NCP_STREAM_PORT );
#else
	sockpas.Bind();
#endif
	...
	sockpas.Listen();
	CSockAddrIn pasaddr;
	sockpas.GetSockName( pasaddr );
	m_nPasPort = pasaddr.GetPort();
	...
	int idLoginTimer = timers.SetTimer( 30*1000, tmLastTick );
	...
	try{
		while( true ){
			...
			FD_SET( (SOCKET)sockpas, &rfds );
			int maxfds = max( (SOCKET)sockpas, max( (SOCKET)sockpoll, (SOCKET)sockacpt ) );
			...
			if( timers.IsTimeOut( idPollTimer, tmNow ) && IsAsProxy() ){
				if( !m_strHostFile.empty() )LoadHostFile( m_strHostFile.c_str() );
				DoPoll( sockpoll );
			}

			//check logins when login timer is time out.
			if( timers.IsTimeOut( idLoginTimer, tmNow ) && IsAsServer() ){
				CheckLogin();
			}
			...
			if( nRet==0 ){	//select timeout, should I do a poll for the remote hosts?
				...
				SOCKADDR_IN srcaddr;
				CMessageBase* pMsg = GetMessage( sockpoll, srcaddr );
				if( pMsg==NULL )break;

	//			cdump<<inet_ntoa(srcaddr.sin_addr)<<" with message id:"<<pMsg->GetMessage()<<endl;
				CMessageTrash trash(pMsg);
				ProcessMessage( pMsg, sockpoll, srcaddr );

			}else if( FD_ISSET( (SOCKET)sockacpt, &rfds) ){
				//client is connecting session socket, accept and start a session.
				CSocket sockclt;
				sockacpt.Accept( sockclt );

				CServerProxy* pServProxy = CreateServerProxy( sockclt.Detach() );
				if( pServProxy )pServProxy->StartServerProxy();
			}
			//data stream connection...
			if( FD_ISSET((SOCKET)sockpas, &rfds) ){
...
void CServerApp::AckPoll( SOCKET sock, SOCKADDR_IN& addr ){
	CObjLocker<CServerApp> locker(this);
	//note. suspended service doesn't advisertise it's tokens.
	int nAckTokens = IsSuspended() ? 0 : m_ServTokens.size();
	int nLength = sizeof(CMessageBase)+ nAckTokens*sizeof(TOKEN_INFO);
	CMessageBase* pMsg = CMessageBase::Alloc( nLength );
	TOKEN_INFO* pinfo = (TOKEN_INFO*)(pMsg+1);
	CMessageTrash trash(pMsg);

	pMsg->Init( NCF_ACK+NCM_POLLSERVER, GetAddr(), CSessionAddr(addr.sin_addr, 0), nAckTokens );
	if( nAckTokens>0 ){
		map<CSessionAddr, CProxyToken*>::iterator iter;
...
CMessageBase* CServerApp::GetMessage( SOCKET sock, SOCKADDR_IN& srcaddr )
{
	...
	if( pMsg->GetMessage()==NCM_SHUTDOWN ){
		CMessageTrash trash(pMsg);
		if( pMsg->GetResult()!=0 && IsAsProxy() ){
			DoForwardShutdown( sock );
		}
		return NULL;
	}
	return pMsg;
...
void CServerApp::DoForwardShutdown( SOCKET sock )
{
	CObjLocker<CServerApp> locker(this);
	...
	for( pos=m_HostAddrs.begin(); pos!=m_HostAddrs.end(); ++pos ){
		CMessageBase msg;
		msg.Init( NCM_SHUTDOWN, GetAddr(), CSessionAddr(*pos, 0), true );
		CSockAddrIn dstaddr( *pos, NCP_SERVER_PING_PORT );
		SendMessageTo( sock, &msg, dstaddr );
	}
}

void CServerApp::ProcessMessage( CMessageBase* pMsg, SOCKET sock, const SOCKADDR_IN& srcaddr )
{
	bool bFromLocal = (GetInAddr()==srcaddr.sin_addr);

	if( pMsg->GetMessage()==NCM_POLLSERVER ){			//a client or proxy is polling me, ACK the poll with tokens
		UINT nPort = pMsg->GetResult();
		//do not respond to polling from itself (broadcast when starting)
		if( !( bFromLocal && nPort==NCP_SERVER_PING_PORT ) ){
			CSockAddrIn addr(srcaddr.sin_addr, nPort);
			AckPoll( sock, addr );
//			AckPoll( socksrv, CSockAddrIn(srcaddr.sin_addr, pMsg->GetResult()) );
		}

	}else if( pMsg->IsAcking(NCM_POLLSERVER) ){			//the server is acking our poll
		if( !bFromLocal )OnAckPoll( pMsg );

	}else if( pMsg->GetMessage()==NCM_SERVERSTART ){	//the server is starting, poll the server
		if( IsAsProxy() && !bFromLocal ){
			CMessageBase msg;
			msg.Init( NCM_POLLSERVER, CSessionAddr(), CSessionAddr(), NCP_SERVER_PING_PORT );
			CSockAddrIn addr(srcaddr.sin_addr, NCP_SERVER_PING_PORT);
			SendMessageTo( sock, &msg, addr );
...
/*int CServerApp::ServiceThread( void* arg )
{
	CServerApp* pServApp = (CServerApp*)arg;
...
//		msg.Broadcast( sockbroad, NCP_SERVER_PING_PORT );
	...
	while( true ){
		...
		double tmNow = GetMilliTime();
		//do poll when poll timer is time out.
		if( timers.IsTimeOut( idPollTimer, tmNow ) && pServApp->IsAsProxy() ){
			if( !pServApp->m_strHostFile.empty() )pServApp->LoadHostFile( pServApp->m_strHostFile.c_str() );
			pServApp->DoPoll( sockpoll );
		}

		//check logins when login timer is time out.
		if( timers.IsTimeOut( idLoginTimer, tmNow ) && pServApp->IsAsServer() ){
			pServApp->CheckLogin();
		}
		...
		if( nRet==0 ){	//select timeout, should I do a poll for the remote hosts?
			...
			SOCKADDR_IN srcaddr;
			CMessageBase* pMsg = pServApp->GetMessage( sockpoll, srcaddr );
			if( pMsg==NULL )break;

//			cdump<<inet_ntoa(srcaddr.sin_addr)<<" with message id:"<<pMsg->GetMessage()<<endl;
			CMessageTrash trash(pMsg);
			pServApp->ProcessMessage( pMsg, sockpoll, srcaddr );
		}
	}
	sockpoll.Close();
...
void CServerApp::CheckLogin()
{
#ifdef _WIN32
	return;
#elif _SHARE_PRIORITY<=2
	...
	if( bMoreLogon ){
		...
		SetSysLogonBit( true );
#else
		SetSysSuspendBit( true );
#endif
	}else{
#if _SHARE_PRIORITY==2
		SetSysLogonBit( false );
#else
		SetSysSuspendBit( false );
#endif
	}
#endif	//#endif _WIN32
}
...
bool CServerApp::MessageFromStream( SOCKET sock )
{
	CMessageBase* pMsg=NULL;
	try{
		pMsg = RecvMessage<CMessageBase>( sock );
	...
	if( pMsg!=NULL ){
		...
		CObjLocker<CServerApp> locker(this);
		{
			list<CServerProxy*>::iterator pos;
			for( pos=m_ServerProxies.begin(); pos!=m_ServerProxies.end(); ++pos ){
				pSession = (*pos)->GetSession( pMsg->GetDstAddr() );
