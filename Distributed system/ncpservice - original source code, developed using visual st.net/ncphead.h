...
#define E_PROXY_TOKEN		0x80000003			//the token is unknown at the proxy
...
#define E_THREAD			0x80020003			//the server can't start a thread
...
#define E_OPENSTREAM		0x80080001			//can't get the stream channel for data transmission

#define E_BROKENPIPE		0x80090000			//message can't reach destination because client server connection is broken
#define E_SERVER_SUSPEND	0x80090001			//the server is suspended and can't accept session request
...
#define NCM_SHUTDOWN		(NCM_SERVERSHUTDOWN)
...
#define E_INCOMPLETE_MSG		1001		//the received incoming message is incomplete
...
MSG_TYPE* RecvMessage( SOCKET sock )
{
	...
	int nRet = RecvBuffer( sock, (char*)&msgBase, sizeof(msgBase) );
	if( nRet==0 )return NULL;			//the socket is closed.
	if( nRet<sizeof(msgBase) )throw new CRemoteException( E_INCOMPLETE_MSG );

	ASSERT( nRet==sizeof(msgBase) );
	int nLen = ntohl( msgBase.GetLength() );
	ASSERT( nLen>=sizeof(msgBase) );

	//now receive the message stream indicated by nLen
	CMessageBase* pMsg = CMessageBase::Alloc( nLen );
	memcpy( (void*)pMsg, &msgBase, sizeof(msgBase) );
	try{
		int nBufLen = nLen - sizeof(msgBase);
		if( nBufLen>0 ){
			nRet = RecvBuffer( sock, ((char*)pMsg+sizeof(CMessageBase)), nBufLen );
			if( nRet<nBufLen )throw new CRemoteException( E_INCOMPLETE_MSG );
			ASSERT( nRet==nBufLen );
		}
...
MSG_TYPE* RecvMessage( SOCKET sock )
{
	...
	int nRet = 0;
	while( true ){
		nRet = recv( sock, (char*)&msgBase, sizeof(CMessageBase), MSG_PEEK );
		if( nRet == 0 )return NULL;
		if( nRet<0 )throw new CSockException();
		if( nRet==sizeof(CMessageBase) )break;
		cdump<<lock<<"RecvMessage incomplete "<<nRet<<endl<<unlock;
		MilliSleep( 20 );
	}
...
//busy flag bit. 
//0 - this token is idle (in the server token set)
//1 - this token is busy (is used by a session)
#define CTF_IDLE		0x00000000
#define CTF_BUSEY		0x80000000

//sound flag bit.
//0 - this token is sound, can be connected.
//1 - this token is dead, either the sever host is unreachable or the token at the server was allocated.
#define CTF_SOUND		0x00000000
#define CTF_DEAD		0x40000000

//the token is dead because the sock connecting the server has error, server host is unreachable..
...
public:
	...
	int				m_nState;
public:
	CServerToken( const CSessionAddr& addrServ, int nCpuType, int nCpuMhz, int nState=CTF_IDLE|CTF_SOUND )
	...
	bool IsIdle(){ return !isset(m_nState, CTF_BUSEY); }
	bool IsBusey(){ return isset(m_nState, CTF_BUSEY); }
	
	bool IsSound(){ return !isset(m_nState, CTF_DEAD); }
	bool IsDead(){ return isset(m_nState, CTF_DEAD); }

	bool IsHostDead(){ return (m_nState==(CTF_DEAD|CTS_E_SOCK)); }

	void SetIdle(){ clrbit(m_nState, CTF_BUSEY); }
	void SetBusey(){ setbit(m_nState, CTF_BUSEY); }
	void SetSound(){ clrbit(m_nState, CTF_DEAD); }
	void SetDead(){ setbit(m_nState, CTF_DEAD); }
	void SetDead( int reason ){ setbit(m_nState, reason|CTF_DEAD); }
	void SetState( int nState ){ m_nState = nState; }
...
public:
	CProxyToken( const CSessionAddr& addrServ, const CSessionAddr& addrPeer, int nCpuType, int nCpuMhz, int nState=CTF_IDLE|CTF_SOUND )
