...
implicit none
	...
	do i=2, nTournSize
		...
		do while ((nSelSize>1) .and. (iMate==iPick) )
			iMate = 1+dint(dble(nSelSize-1)*UniRand())
		enddo
	...
	do i=1, sgaParam%nPopSize
		if( nPdeCount<nMaxPdes )then
			if( (sgaParam%arrFlags(i)==NN_EVAL) .and. (UniRand()<exp(-1.0/nInitSampling*i)) )then
				arrPdeIds(nPdeCount+1) = arrIndex(i)
...
implicit none                                                                                                                                                                                             
	...
	type(NORMALIZEPARAM) :: normHead, normRisk										!normalize parameters
	
                                                                                                                                                                                                          
	!variable                                                                                                                                                                                         
	integer :: i, k, id, hHeadNet, hRiskNet, nPdeCount                                                                                                                                                    
...
!	integer, dimension( sgaParam%nPopSize*2, sgaParam%nChromBits ) :: arrChromPool
	...
	call AssocNormalizeParam( arrOrigMin, arrOrigMax, arrHeadNormMin, arrHeadNormMax, normHead )                                                                                                      
	call AssocNormalizeParam( arrOrigMin, arrOrigMax, arrRiskNormMin, arrRiskNormMax, normRisk )                                                                                                      
	normHead%nLen = 4*g_nRemWells                                                                                                                                                                     
	normRisk%nLen = 4*g_nRemWells+2                                                                                                                                                                   
...
!		call system( 'tool -strip 75 risktest.dat' );
		rHeadMSE = TrainMatlabNet( strHeadM, strHeadTrain, strHeadNN, strHeadTest )
		rRiskMSE = TrainMatlabNet( strRiskM, strRiskTrain, strRiskNN, strRiskTest )
...
!		enddo
	...
	implicit none                                                                                                                                                                                     
		...
		if( sgaParam%arrFlags(id)==0 )then		!neural network
		...
		else if( sgaParam%arrFlags(id)==2 )then	!from cache
			...
			write( hGaOut, fmt=1076, advance="NO" ) id, arrFits(id, g_nRemWells+6), arrFits(id, g_nRemWells+3), arrFits(id, g_nRemWells+4), arrFits(id, g_nRemWells+5), arrFits(id, g_nRemWells+1), arrFits(id, g_nRemWells+2)
	...
	subroutine LoadNets( hHeadNet, hRiskNet )                                                                                                                                                         
	use neuroio                                                                                                                                                                                       
	use nnsgahelper                                                                                                                                                                                   
	implicit none                                                                                                                                                                                     
		integer, intent(out) :: hHeadNet, hRiskNet                                                                                                                                                
		...
		hHeadNet = CreateNet( netParam )                                                                                                                                                          
                                                                                                                                                                                                          
		!setup risk net                                                                                                                                                                           
		layerParam(1)%nNeuros = 12                                                                                                                                                                
		layerParam(2)%nNeuros = 15                                                                                                                                                                
		layerParam(3)%nNeuros = 2                                                                                                                                                                 
                                                                                                                                                                                                          
		hRiskNet = CreateNet( netParam )                                                                                                                                                          
	end subroutine                                                                                                                                                                                    
