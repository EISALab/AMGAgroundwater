...
//	cdump<<lock<<nId<<"--check point 3"<<endl<<unlock;
	...
	pSession->RunImage( cmd, true );

//	cdump<<lock<<nId<<"--check point 4"<<endl<<unlock;
	//retrieve surrback.dat
	strcpy( buf, path );
	strcat( buf, "/surrbak.dat" );
	pSession->GetFile( "surrbak.dat", buf );
	pSession->ChDir( "~" );

//	cdump<<lock<<nId<<"--check point done"<<endl<<unlock;
	return 0;
}
