...
character(*), parameter :: strUmaTest = 'umatest.dat'
character(*), parameter :: strUmaNN = 'umanneu.dat'
character(*), parameter :: strUmaM = 'univnet'
character(*), parameter :: strCostTrain = 'umacosttr.dat'
character(*), parameter :: strCostTest  = 'umacostts.dat'
character(*), parameter :: strCostNN = 'umacostnn.dat'
                                                                                                                                                                                                          
integer :: hChromCache
integer :: totalPDE = 0

integer, dimension(3) :: arrUmaNet = (/28, 20, 3/)
integer, dimension(3) :: arrCostNet = (/28, 20, 2/)
...
implicit none                                                                                                                                                                                             
	...
	!variable                                                                                                                                                                                         
	integer :: i, j, k, id, hUmaNet, hCostNet, nPdeCount
...
!	nMaxNewPoints = 10
	...
	if( sgaParam%iCurGen==1 )then
		...
		i=RemoveFileC( strUmaTest )
		i=RemoveFileC( strUmaNN )
		i=RemoveFileC( strCostTrain )
		i=RemoveFileC( strCostTest )
		i=RemoveFileC( strCostNN )
...
!	hUmaNet = CreateNet( (/28, 20, 2/), 3 );
!	hCostNet = CreateNet( (/28, 20, 2/), 3 );
	hUmaNet = CreateNet( arrUmaNet, 3 )
	hCostNet = CreateNet( arrCostNet, 3 )
	inquire( FILE=strUmaNN, EXIST=bUmaNetOk )
	inquire( FILE=strCostNN, EXIST=bCostNetOk )
	...
	if( bUmaNetOk )then
		...
		call LoadMatlabNet( hUmaNet, strUmaNN )
		bNNReady = .true.
	endif
	if( bCostNetOk )call LoadMatlabNet( hCostNet, strCostNN )
	...
	if( bUmaNetOk )then
		do i=1, sgaParam%nPopSize
			...
			!call neural network
			call SimNet( hUmaNet, arrVars, arrVars(nInputs+1:nInputs+3) )
			call SimNet( hCostNet, arrVars, arrVars(nInputs+4:nInputs+5) )
...
!		if( mod(nPdeCount,4)/=0 )then
		call Normalize( hNormParam, arrVars, 1 )
		if( .not. bSkipSave )call AppendDataset( mod(nNewPoints,4)/=0 )
!		if( mod(nNewPoints,4)/=0 )then
!			strUmaFile = strUmaTrain
!			strCostFile = strCostTrain
!		else                                                                                                                                                                              
!			strUmaFile = strUmaTest
!			strCostFile = strCostTest
!		endif                                                                                                                                                                             
...
!		call AppendToFile( strCostFile, arrVarsTmp, nInputs+2 )
		...
		if( bUmaNetOk )then
			...
			!call Normalize( hNormParam, arrVars, 1 )
			rUmaMSE = rUmaMSE + TestNet( hUmaNet, arrVars, arrVars(nInputs+1:nInputs+3), 1 )
			rCostMSE = rCostMSE + TestNet( hCostNet, arrVarsTmp, arrVarsTmp(nInputs+1:nInputs+2), 1 )
...
!	enddo
	...
	if( bRetrainNN )then
		if( sgaParam%iCurGen<5 )then
			arrUmaNet(2) = 8
			arrCostNet(2) = 8
		else if( sgaParam%iCurGen<30 ) then
			arrUmaNet(2)=15
			arrCostNet(2) = 15
		else 
			arrUmaNet(2) = 15
...
!		call system( 'tool -strip 1000 umatrain.dat' );
!		call system( 'tool -strip 300 umatest.dat' );
		rUmaMSE = TrainMatlabNetEx( strUmaM, arrUmaNet, 3, (/0.d0,0.d0,0.d0,0.0d0/), strUmaTrain, strUmaNN, strUmaTest )
		call ReleaseNet( hUmaNet )
		hUmaNet = CreateNet( arrUmaNet, 3 )
		call LoadMatlabNet( hUmaNet, strUmaNN )
...
!		call system( 'tool -strip 800 umacosttr.dat' );
!		call system( 'tool -strip 200 umacostts.dat' );
		rCostMSE = TrainMatlabNetEx( strUmaM, arrCostNet, 3, (/dble( (log10(2.1*1.05)+0.3)/1.7*1.8-0.9 ), dble( (log10(2.8*1.05)+0.3)/2.2*1.8-0.9 ), 1.2d0/), strCostTrain, strCostNN, strCostTest )
		call ReleaseNet( hCostNet )
		hCostNet = CreateNet( arrCostNet, 3 )
		call LoadMatlabNet( hCostNet, strCostNN )
		...
		do i=1, 10                                                                                                                                                                                
			...
			call SimNet( hUmaNet, arrVars, arrVars(nInputs+1:nInputs+3) )
			call SimNet( hCostNet, arrVarsTmp, arrVarsTmp(nInputs+1:nInputs+2) )
